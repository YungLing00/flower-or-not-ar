...
async function scan(){if(!video.videoWidth)return;const side=Math.min(video.videoWidth,video.videoHeight),sx=(video.videoWidth-side)/2,sy=(video.videoHeight-side)/2;ctx.drawImage(video,sx,sy,side,side,0,0,256,256);const f=featuresFromCanvas(canvas);const ds=targetFeatures.map(t=>dist(f,t));let best=0;for(let i=1;i<ds.length;i++)if(ds[i]<ds[best])best=i;const confidence=Math.max(0,Math.min(99,Math.round((1-ds[best]*3.2)*100)));document.getElementById('resultTitle').textContent='最接近：任務 '+(best+1);document.getElementById('resultConfidence').textContent='辨識信心 '+confidence+'%';result.classList.add('show');if(confidence>=48){statusEl.textContent='辨識成功，正在進入小遊戲…';setTimeout(()=>location.href='game'+(best+1)+'.html',700)}else statusEl.textContent='辨識不足，請靠近並讓圖卡填滿掃描框'}
document.getElementById('scanBtn').onclick=scan;document.querySelectorAll('[data-target]').forEach(b=>b.onclick=()=>location.href='game'+(Number(b.dataset.target)+1)+'.html');
